...
public class TutorialExamples {
    ...
    public static AGRepositoryConnection example1(boolean close)
        ...
        println("\nStarting example1().");
        // tao doi tuong may chu tren may khach
        AGServer server = new AGServer(SERVER_URL, USERNAME, PASSWORD);
        try {
        	// lay thong tin cua server
        ...
        // tao doi tuong catalog tren may khach voi catalog dc khai bao o tren
        ...
        // lay ten catalog va danh sach kho trong catalog
        println("Available repositories in catalog " + 
                (catalog.getCatalogName()) + ": " + 
                catalog.listRepositories());
        // dong tat ca cac kho dang mo va xoa trang thai truoc khi tiep tuc
        // thuc te thi ko can thiet
        closeAll();
        // xoa kho voi repository_id duoc khai bao o tren
        catalog.deleteRepository(REPOSITORY_ID);
        // tao doi tuong kho ben phia may khach voi repository_id duoc khai bao o tren
        AGRepository myRepository = catalog.createRepository(REPOSITORY_ID);
        println("Got a repository.");
        // kho moi thi phai duoc khoi tao
        myRepository.initialize();
        println("Initialized repository.");
        // kiem tra xem kho co the viet duoc hay ko
        println("Repository is writable? " + myRepository.isWritable());
        // tao doi tuong ket noi phia may khach
        AGRepositoryConnection conn = myRepository.getConnection();
        // them doi tuong ket noi vao danh sach cac doi tuong ket noi va
        // se duoc don sach neu may khach thoat
        closeBeforeExit(conn);
        println("Got a connection.");
        // lay ten repository va lay ra so statements
        println("Repository " + (myRepository.getRepositoryID()) +
                " is up! It contains " + (conn.size()) +
                " statements."              
                );
        // lay ra danh sach chi so bo 3 co the co
        List<String> indices = conn.listValidIndices();
        println("All valid triple indices: " + indices);
        // lay ra chi so hien tai (co 7 chi so mac dinh)
        indices = conn.listIndices();
        println("Current triple indices: " + indices);
        println("Removing graph indices...");
        // xoa bo cac chi so bo 3 khong dung den (chi so do thi) de tranh lang phi CPU va khong gian dia
        ...
        if (close) {
        	...
        	// dong ket noi
            conn.close();
            // tat kho de giai phong tai nguyen
            myRepository.shutDown();
            server.close();
            return null;
        }
        // can dung ket noi thi close = false
        return conn;
    }
